    ...
    const loadActivities = () => {
         ...
         const handleSubmit = async (e: React.FormEvent)=>{
            ...
            if(!formData.name.trim() || formData.duration <= 0){
                return toast("Please enter valid date")
            }
            try {
        ...
        <div className="page-container">
            ...
            <div className="page-header">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className={`text-2xl font-semibold capitalize ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Activity Log</h1>
            ...
            <div className="page-content-grid">
                        ...
                        <Card>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {quickActivities.map((activity) => (
                                    <button onClick={() => handleQuickAdd(activity)} key={activity.name} className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors flex items-center gap-2 shadow-sm ${theme === 'dark' ? 'bg-slate-800 hover:bg-slate-700 text-slate-100' : 'bg-white hover:bg-gray-50 text-slate-900'}`}>
                ...
                {showForm && (
                    <Card className="border-2 border-blue-200 dark:border-blue-800">
                        <h3 className={`font-semibold mb-3 capitalize ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>New Activity</h3>
                ...
                {activities.length === 0 ? (
                    <Card className="text-center py-12">
                        <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center mx-auto mb-4">
                          <DumbbellIcon className="w-3 h-3 text-slate-400 dark:text-slate-500"/>
                        </div>
                        <h3 className="">No activities logged today</h3>
                        <p>Start moving and track your progress</p>
                    </Card>
                ) : (
                    <Card>
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 className={`font-semibold capitalize ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Today's Activity</h3>
                            ...
                            {activities.map((activity)=>(
                                <div key={activity.id} className="activity-entry-item">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className={`font-medium capitalize ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{activity.name}</p>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <button onClick={() => handleDelete(activity.documentId)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hoverbg-red-900/20 rounded-lg transition-colors">
